...
contract STT is STTInterface {
	...
	address public contractRoot = address(0);
	uint8 public W = 5;
	uint256 public D = 1;
	...
	function initialize(string _name, string _symbol, uint8 _W, uint256 _D, address _contractRoot) external ownerOnly {
	...
	function getReserve() public constant returns(uint256) {
		...
		if(contractRoot == address(0)) {
			return this.balance;
		} else {
			uint256 rootPrice = STTInterface(contractRoot).getPrice();
			uint256 balance = STTInterface(contractRoot).balanceOf(this);
			return rootPrice * balance;
		}
	}
	...
	function() public payable {
		if(contractRoot == address(0)) {	
			...
			STTInterface(contractRoot).buyTokens.value(msg.value)(address(this));
		}		
	}
	...
	function sellTokens(uint256 _amount, address recipient) external returns(bool) {
		...
		if(contractRoot == address(0)) {
			...
			STTInterface(contractRoot).sellTokens(deltaR, recipient);
		}
	...
	function buyTokens(address recipient) external payable returns(bool) {
		...
		if(contractRoot == address(0)) {	
			...
			newS = SfromR(getReserve() + msg.value);
			balances[msg.sender] = balances[msg.sender] + newS;
			totalSupply += newS;
			STTInterface(contractRoot).buyTokens.value(msg.value)(address(this));
		}
